...
let movies = new Array(vampire, singham, bulk, wars);
console.log(movies);

movies.forEach((movie) => {
	console.log(movie.name);
	let actors = movie.generator();
	let button = document.createElement("button");

	let textButton = document.createTextNode(movie.name);
	button.appendChild(textButton);
	button.addEventListener("click", function () {
		try {
			let nextActor = actors.next().value;
			if (nextActor === undefined) throw "No more actors for " + movie.name;
			let pTag = document.createElement("p");
			let pText = document.createTextNode(nextActor);
