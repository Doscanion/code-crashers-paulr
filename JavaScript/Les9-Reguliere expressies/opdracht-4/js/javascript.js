document.querySelector("button").addEventListener("click", function () {
	...
	const patterntel = /^06\s{1}\d{8}$|^06\-\d{8}$|^06\d{8}$|^\+31\s{1}6\s{1}\d{8}$/;
	console.log(patterntel);
	if (patterntel.test(telNumber) !== true) {
		if (!document.querySelector("#form").contains(document.querySelector("#number-warn"))) {
			const falseNumber = document.createElement("p");
			falseNumber.setAttribute("id", "number-warn");
			falseNumber.style.color = "red";
			const falseNumberText = document.createTextNode("Vul een correct tel. nummer in");
		...
		if (document.querySelector("#form").contains(document.querySelector("#number-warn"))) {
			document.querySelector("#form").removeChild(document.querySelector("#number-warn"));
		}
		console.log("yup");
	}
	...
	if (patternDate.test(dateValue) !== true) {
		if (!document.querySelector("#form").contains(document.querySelector("#date-warn"))) {
			const falseDate = document.createElement("p");
			falseDate.setAttribute("id", "date-warn");
			falseDate.style.color = "red";
			const falseDateText = document.createTextNode("Vul een correct datum in");
		...
		if (document.querySelector("#form").contains(document.querySelector("#date-warn"))) {
			document.querySelector("#form").removeChild(document.querySelector("#date-warn"));
		}
	}
	//|[A-Z]?[a-z]+\s{1}\d[A-Z]?[a-z]$|[A-Z]?[a-z]+\s{1}\d\-[A-Z]?[a-z]$
	const houseValue = document.querySelector("#street").value;
	const patternHouse = /[A-Z]?[a-z]+\s{1}\d{1,5}\-?[A-Z]?[a-z]?$/;
	if (patternHouse.test(houseValue) !== true) {
		if (!document.querySelector("#form").contains(document.querySelector("#house-warn"))) {
			const falseHouse = document.createElement("p");
			falseHouse.setAttribute("id", "house-warn");
			falseHouse.style.color = "red";
			const falseStreetText = document.createTextNode("Vul een correct straat in");
		...
		if (document.querySelector("#form").contains(document.querySelector("#house-warn"))) {
			document.querySelector("#form").removeChild(document.querySelector("#house-warn"));
		}
	}
	...
	if (patternLink.test(pageValue) !== true) {
		if (!document.querySelector("#form").contains(document.querySelector("#page-warn"))) {
			const falsePage = document.createElement("p");
			falsePage.setAttribute("id", "page-warn");
			falsePage.style.color = "red";
			const falsePageText = document.createTextNode("Vul een correct webpage in");
		...
		if (document.querySelector("#form").contains(document.querySelector("#page-warn"))) {
			document.querySelector("#form").removeChild(document.querySelector("#page-warn"));
		}
	}
	const passwordValue = document.querySelector("#password").value;
	const patternPassword = /(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*(?:\!+|\@+|\#+\$+|\%+|\&+|\*+))/;
	if (patternPassword.test(passwordValue) !== true) {
		if (!document.querySelector("#form").contains(document.querySelector("#pass-warn"))) {
			const falsePassword = document.createElement("p");
			falsePassword.setAttribute("id", "pass-warn");
			falsePassword.style.color = "red";
			const falsePasswordText = document.createTextNode("Vul een correct password in");
		...
		if (document.querySelector("#form").contains(document.querySelector("#pass-warn"))) {
			document.querySelector("#form").removeChild(document.querySelector("#pass-warn"));
		}
	}
});
