...
if (isset($_POST['save'])) {
    setcookie('shoppingCart', json_encode($_SESSION['shoppingCart']), time() + ((3 * 24 * 60 * 60)));
}
if (isset($_COOKIE['shoppingCart'])) {
    $_SESSION['shoppingCart'] = json_decode($_COOKIE['shoppingCart'], true);
}
...
if (!empty($_GET['addToCart'])) {
    $existGame = false;
    foreach ($_SESSION['shoppingCart'] as $key => &$cartGame) {
        if ($cartGame['name'] === $_GET['addToCart']) {
            $cartGame['quantity']++;
            echo $cartGame['name'];
            $existGame = true;
        }
        ...
        <main>
            ...
            ?>
                ...
                <table>
                    ...
                    <?php
                    foreach ($_SESSION['shoppingCart'] as $key => $productGame) {
                    ?>
                        <tr>
                            <td><?= $productGame['name'] ?></td>
                            <td><?= '€' . $productGame['price'] ?></td>
                            <td><?= $productGame['quantity'] ?></td>
                            <td><?= '€' . $productGame['price'] * $productGame['quantity'] ?></td>
                        </tr>

                    <?php
                        $totalPrice = ($productGame['price'] * $productGame['quantity']) + $totalPrice;
                    }
                    echo '<tr><td colspan="3"></td><td>€' . $totalPrice . '</td></tr></table>';
                    ?>
                    <div>
                        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post">
                            <input type="submit" value="Empty" name="empty">
                        </form>
                        <form action="<?= htmlspecialchars($_SERVER['PHP_SELF']) ?>" method="post">
                ...
                <section id="products">
                    ...
                    ?>
                            <article>
                                ...
                                <a href="index.php?addToCart=<?= $game['name'] ?>">Toevoegen aan winkelwagentje</a>
                            </article>
                        <?php
                        }
                    } else {
                        ...
                        ?>
                            <article>
                                ...
                                <a href="index.php?addToCart=<?= $value['name'] ?>">Toevoegen aan winkelwagentje</a>
                            </article>
                    <?php
                        }
                    }
